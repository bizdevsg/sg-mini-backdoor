            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                try {
                    ...
                    const quill = new Quill(editorElement, {
                        theme: 'snow',
                        placeholder: 'Tuliskan detail spesifikasi produk.',
                        modules: {
                            table: false,
                            toolbar: [
                                [{
            ...
            <div class="grid gap-5">
                ...
                <div>
                    ...
                    <div data-quill-wrapper
                        class="sg-quill-wrapper {{ $errors->has('specs') ? 'rounded-2xl border border-red-400/60 overflow-hidden' : '' }}">
                        <div data-quill-editor></div>
                    </div>
                    <textarea id="specs" name="specs" data-quill-input rows="10"
                        class="w-full rounded-lg border bg-onyx px-4 py-3 text-champagne placeholder:text-smoke focus:border-gold/30 focus:outline-none focus:ring-2 focus:ring-gold/15 {{ $errors->has('specs') ? 'border-red-400/60' : 'border-white/8' }}"
                        placeholder="Tuliskan detail spesifikasi produk." required>{{ old('specs', $produk?->specs) }}</textarea>
                    <p class="mt-2 text-xs text-smoke">Kalau Quill berhasil dimuat, textarea ini otomatis berubah jadi rich text editor lengkap dengan tombol tabel. Klik kanan di tabel untuk menu baris, kolom, merge, dan properti cell.</p>
                    @error('specs')
                        <p class="mt-2 text-sm text-red-300">{{ $message }}</p>
                    @enderror
                </div>
