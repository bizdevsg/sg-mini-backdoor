    ...
    <section class="space-y-6">
        <div class="rounded-[28px] border border-black/8 bg-black/3 px-7 py-6  ">
            <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <h1 class="text-2xl font-semibold text-ivory lg:text-3xl">Kategori Berita</h1>
        ...
        <div class="rounded-2xl border border-black/8 bg-black/3 px-5 py-4">
            <form action="{{ route('berita-categories.index') }}" method="GET"
                    ...
                    <tbody class="divide-y divide-black/5">
                        @forelse ($categories as $category)
                            <tr class="group align-top transition-colors duration-150 hover:bg-black/3">
                                <td class="px-6 py-4">
                                    <p class="font-semibold text-ivory group-hover:text-gold-soft">{{ $category->name }}</p>
                                </td>
                                <td class="px-4 py-4">
                                    <span
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <a href="{{ route('berita.index', $category) }}"
                                            class="inline-flex items-center gap-1.5 rounded-lg border border-black/10 bg-black/5 px-3 py-1.5 text-xs font-medium text-smoke hover:border-black/18 hover:bg-black/8 hover:text-ivory">
                                        ...
                                        <form action="{{ route('berita-categories.destroy', $category) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" data-confirm-submit data-confirm-intent="delete"
                                                ...
                                                class="inline-flex items-center gap-1.5 rounded-lg border border-red-400/25 bg-red-500/8 px-3 py-1.5 text-xs font-medium text-red-700/80 hover:border-red-400/40 hover:bg-red-500/16 hover:text-red-800">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-16 text-center text-sm text-smoke">Belum ada kategori
                                    berita.</td>
                            </tr>
                        @endforelse
                    </tbody>
