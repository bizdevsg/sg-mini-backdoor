                ...
                <div class="space-y-3 motion-safe:motion-preset-slide-right-sm motion-safe:motion-delay-[60ms]">
                    ...
                    <div>
                        <h1 class="text-2xl font-semibold tracking-[-0.04em] text-white lg:text-3xl">
                            Kelola
                            <span
                                class="bg-gradient-to-r from-gold-soft to-champagne bg-clip-text text-transparent">Pengumuman</span>
                        </h1>
                        <p class="mt-2 max-w-xl text-sm leading-6 text-smoke">
                            Kelola informasi publik, berita, dan pengumuman resmi perusahaan untuk ditampilkan di website.
                        </p>
                    </div>
                </div>
            ...
            @if ($informasis->isEmpty())
                ...
                <div class="flex flex-col items-center px-6 py-20 text-center">
                    ...
                    <p class="mt-2 max-w-sm text-sm leading-6 text-smoke">
                        ...
                        @else
                            Mulai dengan menambahkan materi pengumuman pertama.
                        @endif
                    </p>
                    @if (!request('search'))
                        <a href="{{ route('pengumuman.create') }}"
            ...
            @else
                                ...
                                <tr class="group align-top transition-colors duration-150 hover:bg-white/3">
                                    ...
                                    <td class="hidden px-4 py-4 sm:table-cell">
                                        <p class="text-xs font-medium text-champagne/80">
                                            {{ $informasi->created_at?->format('d M Y') ?? '-' }}</p>
                                        <p class="mt-0.5 text-[10px] text-smoke/60">
                                            {{ $informasi->created_at?->format('H:i') ?? '' }}</p>
                                    </td>
                ...
                <div
                    ...
                    <p class="text-xs text-smoke">
                        Menampilkan
                        <span
                            class="font-medium text-champagne/80">{{ $informasis->firstItem() }}–{{ $informasis->lastItem() }}</span>
                        dari <span class="font-medium text-champagne/80">{{ $informasis->total() }}</span> pengumuman
                    </p>
                    <div class="text-sm">
