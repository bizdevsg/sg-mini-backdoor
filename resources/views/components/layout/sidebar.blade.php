@php
    ...
    $theme = $user?->roleTheme() ?? [
        ...
        'bg_glow_1' => 'bg-gold/10',
        'active_sidebar' =>
            'border-gold bg-gradient-to-r from-gold/18 via-gold/8 to-transparent text-gold-soft font-semibold shadow-sm',
    ...
    $isAdminHost = $user?->isAdminHost() ?? false;
    ...
    $contentItems = $isAdminHost
        ? [
            ...
            [
                ...
                'active' => request()->routeIs('berita.*') || request()->routeIs('berita-categories.*'),
            ],
            [
                'label' => 'Ebook',
        ...
        @foreach ($navigationSections as $section)
            @if (!empty($section['heading']))
                <div class="flex items-center gap-2.5 pt-3">
                    <span class="text-[10px] font-bold uppercase tracking-[0.24em] {{ $theme['text_heading'] }}">
                        {{ $section['heading'] }}
                    </span>
