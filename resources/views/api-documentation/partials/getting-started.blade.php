<div>
    <div class="mb-5 overflow-hidden rounded-2xl border border-black/8 bg-black/3">
        <div class="border-b border-black/6 px-5 py-4">
            <p class="text-sm font-semibold text-ivory">Tentang Halaman Ini</p>
        ...
        <div class="grid gap-0 divide-y divide-black/6 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
            ...
            <div class="px-5 py-4">
                ...
                <p class="mt-2 font-mono text-xs text-emerald-700">application/json</p>
                <p class="mt-2 text-xs leading-5 text-smoke">
    ...
    <div class="mb-4 flex items-center gap-3">
        <div
            class="flex h-7 w-7 items-center justify-center rounded-lg border border-emerald-500/25 bg-emerald-500/15 text-xs font-bold text-emerald-600">
            <i class="fa-solid fa-flag-checkered text-[11px]"></i>
        </div>
        <h2 class="text-base font-semibold text-ivory">Getting Started</h2>
        <div class="h-px flex-1 bg-black/6"></div>
    </div>

    <div class="grid gap-4 sm:grid-cols-3">
        <div class="relative overflow-hidden rounded-2xl border border-black/8 bg-black/3 p-5">
            ...
            <div class="mb-4 flex items-center gap-3">
                ...
                <p class="text-sm font-semibold text-ivory">Siapkan API Key</p>
            </div>
            ...
            @if (filled($apiKeyValue))
                <div
                    ...
                    <span class="text-[11px] font-medium text-emerald-700">API key tersedia</span>
                </div>
            @else
                <div
                    class="mt-3 flex items-center gap-2 rounded-xl border border-amber-400/20 bg-amber-400/8 px-3 py-2">
                    <i class="fa-solid fa-triangle-exclamation text-[11px] text-amber-600"></i>
                    <span class="text-[11px] font-medium text-amber-700">API key belum dikonfigurasi</span>
                </div>
            @endif
        </div>
        ...
        <div class="relative overflow-hidden rounded-2xl border border-black/8 bg-black/3 p-5">
            ...
            <div class="mb-4 flex items-center gap-3">
                <div
                    class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-blue-400/25 bg-blue-400/10 text-sm font-bold text-blue-700">
                    2
                </div>
                <p class="text-sm font-semibold text-ivory">Pilih Tipe Client</p>
            </div>
            <p class="text-xs leading-5 text-smoke">
            ...
            <div class="mt-3 space-y-2 rounded-lg border border-black/8 bg-carbon px-3 py-2 font-mono text-[10px]">
                <p class="text-champagne/80">Origin: {{ $webOriginExample }}</p>
                <p class="text-emerald-700/80">{{ $mobileClientHeader }}: {{ $mobileClientValue }}</p>
            </div>
        </div>
        ...
        <div class="relative overflow-hidden rounded-2xl border border-black/8 bg-black/3 p-5">
            ...
            <div class="mb-4 flex items-center gap-3">
                <div
                    class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-purple-400/25 bg-purple-400/10 text-sm font-bold text-purple-700">
                    3
                </div>
                <p class="text-sm font-semibold text-ivory">Hit Endpoint</p>
            </div>
            <p class="text-xs leading-5 text-smoke">
            ...
            <div
                class="mt-3 rounded-lg border border-black/8 bg-carbon px-3 py-2 font-mono text-[10px] text-emerald-600/80">
    ...
    <div class="mt-5 grid gap-4 lg:grid-cols-2">
        <div class="rounded-2xl border border-black/8 bg-black/3 px-5 py-4">
            <p class="text-sm font-semibold text-ivory">Alur Integrasi Singkat</p>
        ...
        <div class="rounded-2xl border border-black/8 bg-black/3 px-5 py-4">
            <p class="text-sm font-semibold text-ivory">Kesalahan Umum Saat Mulai Integrasi</p>
            <div class="mt-3 space-y-3 text-xs leading-5 text-smoke">
                <p><span class="font-medium text-rose-700">API key salah</span> membuat request langsung ditolak.</p>
                <p><span class="font-medium text-rose-700">Origin web tidak sesuai</span> membuat website tidak bisa akses API.</p>
                <p><span class="font-medium text-rose-700">Lupa header mobile app</span> membuat request aplikasi native ikut ditolak.</p>
                <p><span class="font-medium text-amber-700">API dimatikan dari backdoor</span> membuat semua request public API berhenti sementara.</p>
            </div>
        </div>
    </div>
</div>
