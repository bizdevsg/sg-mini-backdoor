    ...
    <section class="space-y-6">
        ...
        <div
            ...
            <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div class="space-y-3 motion-safe:motion-preset-slide-right-sm motion-safe:motion-delay-[60ms]">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        @elseif ($produk->kategori === 'SPA')
                            <span
                                class="inline-flex items-center gap-1.5 rounded-md border border-blue-400/20 bg-blue-400/8 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.1em] text-blue-700/80">
                                <span class="h-1 w-1 rounded-full bg-blue-400/70"></span>
                                Bilateral
                            </span>
                        @else
                            <span
                                class="inline-flex items-center rounded-md border border-black/10 bg-black/5 px-2.5 py-1 text-[10px] font-medium text-smoke">
                                {{ $produk->kategori }}
                            </span>
                        @endif
                        <span
                ...
                {{-- Actions --}}
                <div
                    class="flex items-center gap-2 motion-safe:motion-preset-slide-left-sm motion-safe:motion-delay-[80ms]">
                    <a href="{{ route('produk.index', ['section' => $section]) }}"
                ...
                <div class="overflow-hidden rounded-2xl border border-black/8 bg-black/3">
                    ...
                    @else
                        <div class="flex h-52 flex-col items-center justify-center gap-3 bg-onyx">
                            <div
                                class="flex h-14 w-14 items-center justify-center rounded-2xl border border-black/8 bg-black/5 text-smoke">
                ...
                <div class="rounded-2xl border border-black/8 bg-black/3 p-5">
                    ...
                    <dl class="mt-4 divide-y divide-black/5 text-sm">
                        <div class="flex items-start justify-between gap-4 py-3">
                            ...
                            <dd class="text-right text-ivory">{{ $produk->kategori }}</dd>
                        </div>
                        <div class="flex items-start justify-between gap-4 py-3">
                        ...
                        <div class="flex items-start justify-between gap-4 py-3">
                            ...
                            <dd class="text-right">
                                <span
                                    class="block text-xs font-medium text-ivory">{{ $produk->created_at?->format('d M Y') ?? '-' }}</span>
                                <span
                                    class="block text-[10px] text-smoke/60">{{ $produk->created_at?->format('H:i') ?? '' }}</span>
                            </dd>
                        </div>
                        <div class="flex items-start justify-between gap-4 py-3">
                            ...
                            <dd class="text-right">
                                <span
                                    class="block text-xs font-medium text-ivory">{{ $produk->updated_at?->format('d M Y') ?? '-' }}</span>
                                <span
                                    class="block text-[10px] text-smoke/60">{{ $produk->updated_at?->format('H:i') ?? '' }}</span>
                            </dd>
                        </div>
                    </dl>
                </div>
            ...
            <div class="space-y-4 motion-safe:motion-preset-slide-left-sm motion-safe:motion-delay-[140ms]">
                ...
                <div class="rounded-2xl border border-black/8 bg-black/3 p-6">
                    <div class="mb-4 flex items-center gap-3 border-b border-black/6 pb-4">
                        <div
                            class="flex h-8 w-8 items-center justify-center rounded-xl border border-black/10 bg-black/5 text-smoke">
                            <i class="fa-solid fa-align-left text-xs"></i>
                        </div>
                        <div>
                            <p class="text-[10px] font-semibold uppercase tracking-[0.18em] text-smoke/60">Konten</p>
                            <h3 class="text-sm font-semibold text-ivory">Deskripsi Produk</h3>
                        </div>
                    </div>
