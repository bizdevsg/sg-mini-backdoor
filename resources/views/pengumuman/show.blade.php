                ...
                <div class="rounded-2xl border border-white/8 bg-white/3 p-5">
                    ...
                    <dl class="mt-4 divide-y divide-white/5 text-sm">
                        <div class="flex items-start justify-between gap-4 py-3">
                            ...
                            <dd class="text-right">
                                <span class="block text-xs font-medium text-white">{{ $informasi->created_at?->format('d M Y') ?? '-' }}</span>
                                <span class="block text-[10px] text-smoke/60">{{ $informasi->created_at?->format('H:i') ?? '' }}</span>
                            </dd>
                        </div>
                        <div class="flex items-start justify-between gap-4 py-3">
                            <dt class="shrink-0 text-smoke">Diupdate</dt>
                            <dd class="text-right">
                                <span class="block text-xs font-medium text-white">{{ $informasi->updated_at?->format('d M Y') ?? '-' }}</span>
                                <span class="block text-[10px] text-smoke/60">{{ $informasi->updated_at?->format('H:i') ?? '' }}</span>
                            </dd>
                        </div>
                    </dl>
                </div>
            ...
            <div class="rounded-2xl border border-white/8 bg-white/3 p-6 motion-safe:motion-preset-slide-left-sm motion-safe:motion-delay-[140ms]">
                <div class="mb-4 flex items-center gap-3 border-b border-white/6 pb-4">
                    ...
                    <div>
                        ...
                        <h3 class="text-sm font-semibold text-white">Konten Publikasi</h3>
                    </div>
                </div>
