...
@section('title', 'Edit Kode TradingView')
...
@section('content')
    <section class="space-y-6">
            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div class="space-y-2 motion-safe:motion-preset-slide-right-sm motion-safe:motion-delay-[60ms]">
                    ...
                    <h1 class="text-2xl font-semibold tracking-[-0.04em] text-ivory lg:text-3xl">
                        Edit:
                        <span
                            class="bg-gradient-to-r from-gold-soft to-champagne bg-clip-text text-transparent">{{ $tradingviewSymbol->name }}</span>
                    </h1>
                    <p class="max-w-xl text-sm leading-6 text-smoke">
                        Perbarui nama, symbol_ws, atau symbol_tv untuk kode ini.
                    </p>
                </div>
                <a href="{{ route('tradingview.index') }}"
                    class="inline-flex w-fit items-center gap-1.5 rounded-xl border border-black/10 bg-black/5 px-4 py-2.5 text-sm font-medium text-smoke transition-all duration-200 hover:border-black/18 hover:bg-black/8 hover:text-ivory motion-safe:motion-preset-slide-left-sm motion-safe:motion-delay-[80ms]">
        ...
        <form action="{{ route('tradingview.update', $tradingviewSymbol) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            @include('tradingview-symbol.partials.form', [
                ...
                'confirmMessage' =>
                    'Pastikan Name, symbol_ws, dan symbol_tv sudah benar. Data ini langsung dipakai oleh API publik setelah disimpan.',
