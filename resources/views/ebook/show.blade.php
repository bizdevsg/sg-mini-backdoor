    ...
    <section class="space-y-6">
        <div class="rounded-2xl border border-white/8 bg-white/4 p-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div class="space-y-3">
                    <p class="text-xs font-medium uppercase tracking-[0.18em] text-smoke">Detail ebook</p>
                    <h2 class="text-3xl font-semibold tracking-[-0.04em] text-white">{{ $ebook->title }}</h2>
                ...
                <div class="rounded-2xl border border-white/8 bg-white/4 p-6">
                    ...
                    <dl class="mt-4 space-y-4 text-sm">
                        <div>
                            ...
                            <dd class="mt-1 text-white">{{ $ebook->category?->name ?? '-' }}</dd>
                        </div>
                        <div>
                            <dt class="text-smoke">File</dt>
                            <dd class="mt-1 break-all text-white">{{ $ebook->file }}</dd>
                        </div>
                        <div>
                            <dt class="text-smoke">Dibuat</dt>
                            <dd class="mt-1 text-white">{{ $ebook->created_at?->format('d M Y, H:i') ?? '-' }}</dd>
                        </div>
                        <div>
                            <dt class="text-smoke">Diupdate</dt>
                            <dd class="mt-1 text-white">{{ $ebook->updated_at?->format('d M Y, H:i') ?? '-' }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
