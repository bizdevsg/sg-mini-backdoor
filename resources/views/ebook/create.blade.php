...
@section('content')
    <section class="space-y-6">
            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div class="space-y-2 motion-safe:motion-preset-slide-right-sm motion-safe:motion-delay-[60ms]">
                    ...
                    <h1 class="text-2xl font-semibold tracking-[-0.04em] text-ivory lg:text-3xl">
                        Tambah Ebook
                        <span
                            class="bg-gradient-to-r from-gold-soft to-champagne bg-clip-text text-transparent">{{ $ebookCategory->name }}</span>
                    </h1>
                    <p class="max-w-xl text-sm leading-6 text-smoke">
                        Unggah dokumen PDF ebook baru beserta cover dan deskripsi pendukung.
                    </p>
                </div>
        ...
        <form action="{{ route('ebook.store', $ebookCategory) }}" method="POST" enctype="multipart/form-data"
            class="space-y-6">
            @csrf
