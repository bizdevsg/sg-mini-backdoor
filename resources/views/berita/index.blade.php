...
@section('content')
    ...
    <section class="space-y-6">
        <div class="rounded-[28px] border border-black/8 bg-black/3 px-7 py-6  ">
            <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div class="space-y-2">
                    ...
                    <h1 class="text-2xl font-semibold text-ivory lg:text-3xl">Berita: <span
                            class="{{ $theme['text'] }}">{{ $beritaCategory->name }}</span></h1>
        ...
        <div class="rounded-2xl border border-black/8 bg-black/3 px-5 py-4">
            <form action="{{ route('berita.index', $beritaCategory) }}" method="GET"
                    ...
                    <div class="mt-4 flex items-center justify-between border-t border-black/6 pt-3.5">
                        ...
                        <div class="flex items-center gap-1.5">
                            <a href="{{ route('berita.show', ['beritaCategory' => $beritaCategory, 'berita' => $berita]) }}"
                                class="inline-flex items-center gap-1.5 rounded-lg border border-black/10 bg-black/5 px-2.5 py-1.5 text-xs font-medium text-smoke hover:border-black/18 hover:bg-black/8 hover:text-ivory">
                            ...
                            <form
                                action="{{ route('berita.destroy', ['beritaCategory' => $beritaCategory, 'berita' => $berita]) }}"
        ...
        <div
            ...
            <p class="text-xs text-smoke">
                Menampilkan <span
                    class="font-medium text-champagne/80">{{ $beritas->firstItem() ?? 0 }}-{{ $beritas->lastItem() ?? 0 }}</span>
