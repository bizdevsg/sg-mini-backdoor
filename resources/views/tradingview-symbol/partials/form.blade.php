...
<div class="rounded-2xl border border-white/8 bg-white/3 p-6 space-y-5">
    <div class="border-b border-white/6 pb-4">
        <h3 class="text-base font-semibold text-white">Detail Kode TradingView</h3>
    ...
    <div class="grid gap-5 md:grid-cols-2">
        ...
        <div>
            ...
            <input type="text" id="symbol_ws" name="symbol_ws" value="{{ old('symbol_ws', $tradingviewSymbol?->symbol_ws) }}"
                class="w-full rounded-xl border bg-onyx px-4 py-3 font-mono text-sm text-gold-soft placeholder:text-smoke/40 focus:border-gold/35 focus:outline-none focus:ring-2 focus:ring-gold/12 transition-colors {{ $errors->has('symbol_ws') ? 'border-red-400/60' : 'border-white/8' }}"
        ...
        <div>
            ...
            <input type="text" id="symbol_tv" name="symbol_tv" value="{{ old('symbol_tv', $tradingviewSymbol?->symbol_tv) }}"
                class="w-full rounded-xl border bg-onyx px-4 py-3 font-mono text-sm text-gold-soft placeholder:text-smoke/40 focus:border-gold/35 focus:outline-none focus:ring-2 focus:ring-gold/12 transition-colors {{ $errors->has('symbol_tv') ? 'border-red-400/60' : 'border-white/8' }}"
