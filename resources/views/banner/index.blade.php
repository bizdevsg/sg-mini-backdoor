                ...
                <div class="space-y-3 motion-safe:motion-preset-slide-right-sm motion-safe:motion-delay-[60ms]">
                    ...
                    <div>
                        <h1 class="text-2xl font-semibold tracking-[-0.04em] text-ivory lg:text-3xl">
                            Kelola Banner
                            <span
                                class="bg-gradient-to-r {{ $theme['gradient_text'] }} bg-clip-text text-transparent">Website</span>
                        </h1>
        ...
        <div class="grid gap-4 sm:grid-cols-3 motion-safe:motion-preset-fade-lg motion-safe:motion-delay-[80ms]">
            <div
                ...
                <div>
                    ...
                    <p class="mt-0.5 text-2xl font-semibold text-ivory">{{ $banners->total() }}</p>
                </div>
            </div>

            <div
                class="flex items-center gap-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5 transition-all hover:border-emerald-500/30 hover:bg-emerald-500/8">
                <div
                    class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-emerald-500/25 bg-emerald-500/12 text-emerald-600">
                    <i class="fa-solid fa-circle-check text-lg"></i>
                </div>
                <div>
                    <p class="text-[10px] font-semibold uppercase tracking-[0.18em] text-emerald-600/80">Aktif Tayang</p>
                    <p class="mt-0.5 text-2xl font-semibold text-ivory">{{ $activeCount }}</p>
                </div>
            </div>
            ...
            <div
                class="flex items-center gap-4 rounded-2xl border border-black/8 bg-black/3 p-5 transition-all hover:border-black/14 hover:bg-black/5">
                <div
                    class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-black/10 bg-black/5 text-smoke/60">
                ...
                <div>
                    ...
                    <p class="mt-0.5 text-2xl font-semibold text-ivory">{{ $inactiveCount }}</p>
                </div>
            </div>
        </div>
        ...
        @if ($banners->isEmpty())
            <div
                ...
                <div class="flex flex-col items-center px-6 py-20 text-center">
                    ...
                    <h3 class="mt-6 text-xl font-semibold text-ivory">Belum ada banner</h3>
        ...
        @else
            <div class="grid gap-6 lg:grid-cols-2 motion-safe:motion-preset-slide-up-sm motion-safe:motion-delay-[120ms]">
                @foreach ($banners as $banner)
                    <article
                        class="group relative flex flex-col justify-between overflow-hidden rounded-2xl border border-black/8 bg-black/3 p-5 transition-all duration-300 hover:border-gold/25 hover:bg-black/5 hover:shadow-[0_16px_40px_rgba(0,0,0,0.35)]">
                        <div>
                            {{-- Image Container --}}
                            <div class="relative overflow-hidden rounded-xl border border-black/8 bg-onyx">
                                @if ($banner->image_url)
                                    <img src="{{ $banner->image_url }}" alt="{{ $banner->title }}"
                                        class="h-52 w-full object-cover transition-transform duration-500 group-hover:scale-[1.02]">
                                @else
                                ...
                                <div class="absolute left-3 top-3 flex flex-wrap items-center gap-2">
                                    @if ($banner->is_active)
                                        <span
                                            class="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-950/80 px-3 py-1 text-[10px] font-semibold text-emerald-300 shadow-md backdrop-blur-md">
                                            <span class="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400"></span>
                                            Aktif
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-black/70 px-3 py-1 text-[10px] font-semibold text-white/70 shadow-md backdrop-blur-md">
                                    ...
                                    <span
                                        class="inline-flex items-center gap-1 rounded-full border border-gold/30 bg-black/70 px-2.5 py-1 text-[10px] font-medium text-gold shadow-md backdrop-blur-md">
                            ...
                            <div class="mt-4 space-y-3">
                                <div class="flex items-start justify-between gap-3">
                                    <h3
                                        class="text-lg font-semibold leading-snug text-ivory transition-colors group-hover:text-gold-soft">
                                        {{ $banner->title }}
                                    </h3>
                                    <span
                                        class="shrink-0 rounded-md border border-black/8 bg-onyx px-2 py-0.5 font-mono text-[10px] text-smoke/70">
                                        ID: {{ $banner->id }}
                                    </span>
                                </div>
                                ...
                                {{-- File name pill --}}
                                <div
                                    class="flex items-center gap-2 rounded-xl border border-black/6 bg-onyx px-3 py-2 text-xs text-smoke/70">
                                    <i class="fa-solid fa-file-image shrink-0 text-smoke/50"></i>
                                    <span class="truncate font-mono text-[11px] text-smoke/80"
                                        title="{{ $banner->image }}">{{ $banner->image }}</span>
                                </div>

                                {{-- Terms snippet --}}
                                <div class="rounded-xl border border-black/6 bg-onyx p-3.5">
                                    <p class="text-[10px] font-semibold uppercase tracking-[0.18em] text-smoke/60">Syarat &
                                        Ketentuan</p>
                                    ...
                                    @else
                                        <p class="mt-1.5 text-xs italic text-smoke/40">Belum ada syarat & ketentuan khusus.
                                        </p>
                                    @endif
                                </div>
                            </div>
                        ...
                        {{-- Card Actions Footer --}}
                        <div class="mt-5 flex items-center justify-end gap-2 border-t border-black/6 pt-4">
                            ...
                            <form action="{{ route('banner.destroy', $banner) }}" method="POST" class="inline">
                                ...
                                <button type="submit" data-confirm-submit data-confirm-intent="delete"
                                    ...
                                    class="inline-flex items-center gap-1.5 rounded-lg border border-red-400/25 bg-red-500/8 px-3.5 py-1.5 text-xs font-medium text-red-700/80 transition-all duration-150 hover:border-red-400/40 hover:bg-red-500/16 hover:text-red-800">
            ...
            <div
                ...
                <p class="text-xs text-smoke">
                    Menampilkan <span
                        class="font-medium text-champagne/80">{{ $banners->firstItem() }}–{{ $banners->lastItem() }}</span>
                    dari
