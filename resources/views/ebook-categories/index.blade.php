...
@section('content')
    <section class="space-y-6">
        ...
        <div
            ...
            <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div class="space-y-3 motion-safe:motion-preset-slide-right-sm motion-safe:motion-delay-[60ms]">
                    ...
                    <div>
                        <h1 class="text-2xl font-semibold tracking-[-0.04em] text-white lg:text-3xl">
                            Kategori
                            <span
                                class="bg-gradient-to-r from-gold-soft to-champagne bg-clip-text text-transparent">Ebook</span>
                        </h1>
                        <p class="mt-2 max-w-xl text-sm leading-6 text-smoke">
                            Kelola kategori ebook sebagai pengelompokan utama dokumen publikasi digital.
                        </p>
                    </div>
                </div>
                ...
                <div
                    ...
                    @endif
                    <a href="{{ route('ebook-categories.create') }}"
        ...
        <div
            ...
            <form action="{{ route('ebook-categories.index') }}" method="GET"
                ...
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" id="search" value="{{ request('search') }}"
                        placeholder="Cari nama atau slug kategori ebook..."
        ...
        <div
            ...
            @if ($categories->isEmpty())
                ...
                <div class="flex flex-col items-center px-6 py-20 text-center">
                    ...
                    <h3 class="mt-6 text-xl font-semibold text-white">Belum ada kategori ebook</h3>
                    <p class="mt-2 max-w-sm text-sm leading-6 text-smoke">
                        @if (request('search'))
                            Tidak ditemukan kategori untuk pencarian "<span
                                class="text-champagne/80">{{ request('search') }}</span>".
                        @else
                            Tambahkan kategori terlebih dahulu agar ebook dapat dikelompokkan.
                        @endif
                    </p>
                    @if (!request('search'))
                        <a href="{{ route('ebook-categories.create') }}"
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr
                                ...
                                <th class="px-4 py-3.5 w-fit text-nowrap">Jumlah Ebook</th>
                        ...
                        <tbody class="divide-y divide-white/5">
                            @foreach ($categories as $category)
                                <tr class="group align-top transition-colors duration-150 hover:bg-white/3">
                                    ...
                                    {{-- Slug --}}
                                    <td class="px-4 py-4 w-fit text-nowrap">
                                        <span
                                    ...
                                    <td class="px-4 py-4 w-fit text-nowrap">
                                        <span
                                            class="inline-flex items-center gap-1.5 rounded-md border border-gold/20 bg-gold/10 px-2.5 py-1 text-xs font-medium text-gold-soft">
                                            <i class="fa-solid fa-book-open text-[10px]"></i>
                                            {{ $category->ebooks_count }} Ebook
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-end gap-1.5">
                                            <a href="{{ route('ebook.index', $category->slug) }}"
                                                class="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-nowrap font-medium text-smoke transition-all duration-150 hover:border-white/18 hover:bg-white/8 hover:text-white"
                                                title="Lihat daftar ebook">
                                            ...
                                            <form action="{{ route('ebook-categories.destroy', $category->slug) }}"
                                                method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" data-confirm-submit data-confirm-intent="delete"
                                                    data-confirm-title="Hapus kategori ebook ini?"
                                                    data-confirm-message="Kategori {{ $category->name }} akan dihapus permanen jika sudah tidak dipakai ebook."
