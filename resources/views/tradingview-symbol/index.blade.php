...
@section('title', 'Kode TradingView')
...
@section('content')
    ...
    <section class="space-y-6">
        ...
        <div
            ...
            <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div class="space-y-3 motion-safe:motion-preset-slide-right-sm motion-safe:motion-delay-[60ms]">
                    ...
                    <div>
                        <h1 class="text-2xl font-semibold tracking-[-0.04em] text-ivory lg:text-3xl">
                            Kode
                            <span
                                class="bg-gradient-to-r {{ $theme['gradient_text'] }} bg-clip-text text-transparent">TradingView</span>
                        </h1>
                        <p class="mt-2 max-w-xl text-sm leading-6 text-smoke">
                            Kelola pemetaan simbol antara feed websocket internal dan simbol chart TradingView.
                        </p>
                    </div>
                </div>
                ...
                <div
                    ...
                    @if (!$tradingviewSymbols->isEmpty())
                        <span class="rounded-xl border border-black/8 bg-black/5 px-4 py-2.5 text-sm text-smoke">
                            {{ $tradingviewSymbols->total() }} simbol
                        </span>
                    @endif
                    <a href="{{ route('tradingview.create') }}"
        ...
        <div
            ...
            <form action="{{ route('tradingview.index') }}" method="GET"
                ...
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" id="search" value="{{ request('search') }}"
                        placeholder="Cari nama, symbol_ws, atau symbol_tv..."
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('tradingview.index') }}"
        ...
        <div
            ...
            @if ($tradingviewSymbols->isEmpty())
                <div class="flex flex-col items-center px-6 py-20 text-center">
                    ...
                    <h3 class="mt-6 text-xl font-semibold text-ivory">Belum ada kode TradingView</h3>
                    <p class="mt-2 max-w-sm text-sm leading-6 text-smoke">
                        @if (request('search'))
                            Tidak ditemukan data untuk pencarian "<span
                                class="text-champagne/80">{{ request('search') }}</span>".
                        @else
                            Mulai dengan menambahkan pemetaan simbol pertama.
                        @endif
                    </p>
                    @if (!request('search'))
                        <a href="{{ route('tradingview.create') }}"
            ...
            @else
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr
                                ...
                                <th class="px-4 py-3.5">symbol_ws</th>
                                <th class="px-4 py-3.5">symbol_tv</th>
                                <th class="hidden px-4 py-3.5 lg:table-cell">Dibuat</th>
                                <th class="px-4 py-3.5 text-right">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-black/5">
                            @foreach ($tradingviewSymbols as $item)
                                <tr class="group align-top transition-colors duration-150 hover:bg-black/3">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-gold/20 bg-gold/10 text-gold-soft">
                                    ...
                                    <td class="px-4 py-4">
                                        <span
                                            class="inline-flex items-center gap-1.5 rounded-lg border border-gold/25 bg-gold/10 px-3 py-1 font-mono text-xs font-medium text-gold-soft">
                                            {{ $item->symbol_ws }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-4 py-4">
                                        <span
                                            class="inline-flex items-center gap-1.5 rounded-lg border border-black/10 bg-black/5 px-3 py-1 font-mono text-xs font-medium text-champagne/80">
                                            {{ $item->symbol_tv }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-end gap-1.5">
                                            <a href="{{ route('tradingview.edit', $item) }}"
                                            ...
                                            <form action="{{ route('tradingview.destroy', $item) }}" method="POST"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" data-confirm-submit data-confirm-intent="delete"
                                                    data-confirm-title="Hapus kode TradingView ini?"
                                                    data-confirm-message="Kode {{ $item->name }} ({{ $item->symbol_ws }}) akan dihapus permanen. Tindakan ini tidak bisa dibatalkan."
                ...
                <div
                    ...
                    <p class="text-xs text-smoke">
                        Menampilkan
                        <span
                            class="font-medium text-champagne/80">{{ $tradingviewSymbols->firstItem() }}–{{ $tradingviewSymbols->lastItem() }}</span>
                        dari <span class="font-medium text-champagne/80">{{ $tradingviewSymbols->total() }}</span> kode
                        TradingView
                    </p>
                    <div class="text-sm">
                        {{ $tradingviewSymbols->appends(request()->query())->links() }}
                    </div>
                </div>
            @endif
        </div>
