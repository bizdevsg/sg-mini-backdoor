                ...
                <div class="space-y-3 motion-safe:motion-preset-slide-right-sm motion-safe:motion-delay-[60ms]">
                    ...
                    <div>
                        <h1 class="text-2xl font-semibold tracking-[-0.04em] text-white lg:text-3xl">
                            Kode
                            <span class="bg-gradient-to-r {{ $theme['gradient_text'] }} bg-clip-text text-transparent">TradingView</span>
                        </h1>
        ...
        <div class="overflow-hidden rounded-2xl border border-white/8 bg-white/3 motion-safe:motion-preset-slide-up-sm motion-safe:motion-delay-[120ms]">
            @if ($tradingviewSymbols->isEmpty())
                <div class="flex flex-col items-center px-6 py-20 text-center">
                    ...
                    <h3 class="mt-6 text-xl font-semibold text-white">Belum ada kode TradingView</h3>
            ...
            @else
                                ...
                                <tr class="group align-top transition-colors duration-150 hover:bg-white/3">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <p class="font-semibold text-white transition-colors group-hover:text-gold-soft">{{ $item->name }}</p>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-4">
                                        <span class="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-1 font-mono text-xs font-medium text-champagne/80">
                                            {{ $item->symbol_tv }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-end gap-1.5">
                                            ...
                                            <form action="{{ route('tradingview.destroy', $item) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    data-confirm-submit
                ...
                <div class="flex flex-col items-start justify-between gap-3 border-t border-white/6 bg-noir/30 px-6 py-4 sm:flex-row sm:items-center">
                    <p class="text-xs text-smoke">
                        Menampilkan
                        <span class="font-medium text-champagne/80">{{ $tradingviewSymbols->firstItem() }}–{{ $tradingviewSymbols->lastItem() }}</span>
                        dari <span class="font-medium text-champagne/80">{{ $tradingviewSymbols->total() }}</span> kode TradingView
                    </p>
                    <div class="text-sm">
