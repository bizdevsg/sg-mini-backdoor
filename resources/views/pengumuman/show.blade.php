            ...
            <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div class="space-y-3 motion-safe:motion-preset-slide-right-sm motion-safe:motion-delay-[60ms]">
                    ...
                    <div class="flex items-center gap-2 text-xs text-smoke/60">
                        <a href="{{ route('pengumuman.index') }}" class="transition-colors hover:text-smoke">
                            Pengumuman
                        </a>
        ...
        <div class="grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
            ...
            <div class="space-y-4 motion-safe:motion-preset-slide-right-sm motion-safe:motion-delay-[120ms]">
                ...
                <div class="overflow-hidden rounded-2xl border border-black/8 bg-black/3">
                    ...
                    @else
                        <div class="flex h-48 flex-col items-center justify-center gap-3 bg-onyx">
                            <div
                ...
                <div class="rounded-2xl border border-black/8 bg-black/3 p-5">
                    <p class="text-[10px] font-semibold uppercase tracking-[0.2em] text-smoke/60">Informasi Pengumuman</p>
                    <dl class="mt-4 divide-y divide-black/5 text-sm">
                        <div class="flex items-start justify-between gap-4 py-3">
                            <dt class="shrink-0 text-smoke">Dibuat</dt>
                            <dd class="text-right">
                                <span
                                    class="block text-xs font-medium text-ivory">{{ $informasi->created_at?->format('d M Y') ?? '-' }}</span>
                                <span
                                    class="block text-[10px] text-smoke/60">{{ $informasi->created_at?->format('H:i') ?? '' }}</span>
                            </dd>
                        </div>
                        <div class="flex items-start justify-between gap-4 py-3">
                            ...
                            <dd class="text-right">
                                <span
                                    class="block text-xs font-medium text-ivory">{{ $informasi->updated_at?->format('d M Y') ?? '-' }}</span>
                                <span
                                    class="block text-[10px] text-smoke/60">{{ $informasi->updated_at?->format('H:i') ?? '' }}</span>
                            </dd>
                        </div>
                    </dl>
                </div>
            ...
            <div
                ...
                <div class="mb-4 flex items-center gap-3 border-b border-black/6 pb-4">
                    ...
                    <div>
                        <p class="text-[10px] font-semibold uppercase tracking-[0.18em] text-smoke/60">Isi Pengumuman</p>
                        <h3 class="text-sm font-semibold text-ivory">Konten Publikasi</h3>
                    </div>
                </div>
