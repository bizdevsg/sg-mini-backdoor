@php($beritaCategory = $beritaCategory ?? null)
...
<div class="rounded-2xl border border-black/8 bg-black/3 p-6 space-y-5">
    <div class="border-b border-black/6 pb-4">
        <h3 class="text-base font-semibold text-ivory">Detail Kategori Berita</h3>
        <p class="mt-0.5 text-xs text-smoke">Tentukan kategori untuk mengelompokkan berita.</p>
    </div>
    ...
    <div>
        ...
        <input type="text" id="name" name="name" value="{{ old('name', $beritaCategory?->name) }}"
            class="w-full rounded-xl border bg-onyx px-4 py-3 text-sm text-champagne placeholder:text-smoke/40 focus:border-gold/35 focus:outline-none focus:ring-2 focus:ring-gold/15 transition-colors {{ $errors->has('name') ? 'border-red-400/60' : 'border-black/8' }}"
            placeholder="Contoh: Market Update" required>
